...
## 已知条目判据（`eval/known-item.ts`，2026-09-25 起）

每条 trigger 本身就是一条「问法 → 目标 id」：拿 trigger 原话去搜，看它的宿主排第几。用例在运行时从
`memory_triggers` 侧表生成，零人工标注，trigger 原文不进仓库也不进报告（报告只有 id 与数字）。

```
bun run eval/known-item.ts run                                    # 生产库，只读
bun run eval/known-item.ts run --db <快照 lancedb 目录> --now <毫秒或 ISO> --label <名>   # 快照 + 冻结时钟，可逐字节复现
bun run eval/known-item.ts compare <前一次.json> <后一次.json>    # 救回 / 掉出第 1 / 进出前 10，逐条列出
```

- 只读：检索走 `source: "auto-recall"`（三处强化写入都门控在它上面），审计日志换成空实现；
  `--db` 指向的目录里没有 `memories.lance` 就拒绝运行（`validateStoragePath` 遇到不存在的路径会 mkdir）。
- 输出：第 1 / 前 3 / 前 10 / 前 limit，被 `shouldSkipRetrieval` 整条跳过的单独计数；按宿主存入天数、
  按宿主自身频次倍数分组；第 1 名终分被截平在 1.0 的查询数。
- 用法定位：改评分链时 shadow 判据的一部分——两次 `run`（改前 / 改后）再 `compare`，掉出第 1 的每条都要说明性质。
  它不替代 canary、fixture 与真实查询回归：非 trigger 的日常查询有多少答案本来靠热度上位，这里测不到。
- 边界：只量已知条目召回，量不了答案质量；查询就是 trigger 原文（余弦 1.0 的上限情形），不代表换说法后的召回。
- 基线（2026-09-24 23:39 生产快照，时钟冻结在快照时刻，生产配置）：501 条用例（506 行 trigger，5 行宿主不在主表），
  第 1 190（38%）、前 3 312（62%）、前 10 481（96%）、前 20 499，另 2 条被入口跳过；
  宿主频次台账不到 2 次的第 1 只有 18%，×1.3 以上的 97%。诊断全文与逐环节归因见 open-loops「RecallNest 检索评分链」症状 C。
