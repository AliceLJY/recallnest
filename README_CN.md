...
一目了然地看到记忆的增长 —— 总量、类别分布、健康评分、增长趋势。
...
将记忆网络可视化为交互式力导向图。每个节点是一条记忆，按类别着色。**语义桥接**（粉色线条）揭示隐藏的跨域关联 —— 分散在不同 scope 里、但共享相同底层洞察的知识。
...
| **Knowledge Graph** | 导出交互式 HTML 知识图谱，含跨域语义桥接 |
...
RecallNest 已从简单的 transcript 搜索工具演进为完整的**记忆操作系统** —— 现在以记忆哲学研究为理论基础，从柏拉图到 Michaelian。

| 指标 | 数值 |
|------|------|
| 核心代码 | 35,500+ 行，115+ 个源文件 |
| MCP 工具 | 41 个工具，分 3 个层级（core / advanced / governance） |
| HTTP 端点 | 21 个 REST 端点 |
| 测试覆盖 | 1,391 个测试，0 失败 |
| 检索 | 6 通道混合：向量 + BM25 + L0/L1/L2 多向量 + KG 图（PPR） |
| 记忆演化 | Supersede 链、情绪感知 Weibull 衰减、LLM 重要性评分、自动归档 |
...
- **会话蒸馏** —— 3 层对话压缩（微缩 → LLM 结构化摘要 → 知识提取到持久记忆）
- **对话导入** —— 导入 Claude Code、Claude.ai、ChatGPT、Slack 或纯文本的历史 —— 自动检测格式
- **Topic Tags** —— scope 内 topic 分区，15 个自动检测的 topic，可在 `search_memory` 中过滤
- **轻量唤醒** —— `resume_context(mode='light')` 返回 <300 token，适合低预算终端
- **Skill Memory** —— 存储、检索、自动提升来自重复模式的可执行技能
- **准入控制** —— 写入时门控：噪音过滤、重要性下限、去重、限流
- **Memory Lint** —— 内容质量检查器 + 健康评分（矛盾、重复、过期、孤儿）
- **Knowledge Graph** —— D3.js 力导向图可视化 + 跨 scope 语义桥接

### 架构亮点

RecallNest 不是简单的向量搜索包装器。关键设计决策：

- **Weibull 衰减 + 情绪调制** —— 记忆沿参数化 Weibull 曲线衰减；情绪显著性可额外延长半衰期最高 30%。一次痛苦的三天调试比一次普通配置更改更难被遗忘 —— 就像人类记忆
- **建构式检索** —— 受模拟理论（Michaelian）启发：检索不只是查找存储文本，而是通过扩展 KG 邻居、演化链和叙事兄弟来*重建*上下文，并通过 source-map 溯源和矛盾检测保证可靠性
- **四级隐私 + 级联遗忘** —— `ephemeral` 记忆不留痕迹（无 KG，自动过期）；`private` 持久但不泄漏到知识图谱；`durable` 为默认；`shared` 启用跨 scope 访问。删除级联贯穿所有衍生物，带完整审计日志
- **自传叙事层** —— 记忆携带正交叙事元数据（生命阶段 / 一般事件 / 具体事件），支持"说说那一周的事"式检索，不破坏 6 类别系统
- **L0 / L1 / L2 动态折叠** —— 每条记忆存储 3 个粒度层（一句话 / 要点概要 / 完整内容）；检索时根据相关性分数和 token 预算动态选择返回哪个层级
- **向量预筛 + LLM 去重** —— 90% 的去重决策用低成本余弦相似度（≥ 0.92）；仅临界情况调用 LLM 判断，控制成本的同时不牺牲准确率
- **类别分化合并策略** —— `profile` 和 `preferences` 采用冲突合并（新版覆盖）；`events` 和 `cases` 采用追加（保留历史）。没有一刀切
- **展示分 vs 淘汰分双轨制** —— 检索使用双轨评分：tier floor 防止核心记忆被淘汰，decay boost 让新鲜记忆临时浮现而不永久挤掉稳定记忆
- **预测式前瞻记忆** —— 系统不只回忆过去，还预测你接下来需要什么，从过期待办、工作流模式和行为信号中浮现建议 —— 零 LLM 成本
...
> **前置要求：** 安装 [Bun](https://bun.sh) 运行时。首次启动自动安装依赖。
...
<details>
<summary><strong>MCP 工具（40 个）</strong></summary>
...
| `memory_stats` | 查看索引统计 |
| `memory_drill_down` | 查看某条记忆的完整元数据和来源追踪 |
| `auto_capture` | 零 LLM 调用，从文本中启发式提取并存储记忆信号 |
| `set_reminder` | 设置前瞻性记忆提醒，在未来会话中自动浮现 |
| `consolidate_memories` | 聚类近重复记忆并合并（默认 dry-run，不直接写入） |
| `store_skill` | 存储一个带触发条件和验证的可执行技能 |
| `retrieve_skill` | 通过语义相似度检索匹配的可执行技能 |
| `scan_skill_promotions` | 扫描 cases/patterns 中可提升为 skill 的候选 |
| `list_tools` | 按层级发现可用工具（core/advanced/full） |
| `batch_store` | 单次调用批量存储最多 20 条记忆（带去重） |
| `distill_session` | 通过 3 层管线蒸馏对话为结构化知识 |
| `import_conversations` | 导入 Claude Code、ChatGPT、Slack 等对话 |
| `data_checkup` | 运行数据质量健康检查 |
| `dream` | 离线记忆整合（聚类、合并、修剪） |
| `memory_lint` | 运行记忆质量检查：矛盾、重复、过期、孤儿 |
| `export_graph` | 导出记忆为交互式 HTML 知识图谱 |
...
| `/v1/stats` | GET | 查看记忆统计 |
| `/v1/health` | GET | 健康检查 |
| `/v1/lint` | GET | 运行记忆质量检查，返回健康评分 |
| `/v1/import` | POST | 导入对话历史（Claude Code / ChatGPT / Slack） |
...
# 记忆质量检查
bun run src/cli.ts lint                          # 运行 lint + 输出健康评分
bun run src/cli.ts lint --scope project:myapp    # 按 scope 过滤
bun run src/cli.ts lint --verbose --json         # 详细 + JSON 输出

# 知识图谱
bun run src/cli.ts graph --open                  # 导出并打开知识图谱
bun run src/cli.ts graph --max-nodes 100         # 自定义节点数
```
...
Web UI 提供 **Memory Dashboard**（统计、类别分布、健康评分、增长趋势、过期记忆）和 **Search Workbench**（搜索、解释、蒸馏、Pin、导出）。
