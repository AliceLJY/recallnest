...
RecallNest 已从简单的 transcript 搜索工具演进为完整的**记忆操作系统**：

| 指标 | 数值 |
|------|------|
| 核心代码 | 31,800+ 行，108 个源文件 |
| MCP 工具 | 40 个工具，分 3 个层级（core / advanced / governance） |
| HTTP 端点 | 21 个 REST 端点 |
| 测试覆盖 | 1,168 个测试，0 失败 |
| 检索 | 6 通道混合：向量 + BM25 + L0/L1/L2 多向量 + KG 图（PPR） |
| 记忆演化 | Supersede 链、Weibull 衰减、LLM 重要性评分、自动归档 |
...
- **Weibull 衰减 + Importance 调制** —— 记忆沿参数化 Weibull 曲线衰减；LLM 评估的重要性分数（带锚定评分标准）调节半衰期，核心身份事实持久留存，临时细节自然淡化
...
**全部 6 项胜出或持平**，无任何退步。混合检索管线（BM25 + 向量 + 时效性 + RIF 去重）比纯向量搜索多召回 12.2% 的相关上下文。两个系统都受限于 gpt-4o-mini reader —— 检索质量的真实差距比准确率数字更大。
