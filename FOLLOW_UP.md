...
### P1 待办 — 历史数据清理（低优先，不阻塞）

production 库已有约 87 条 bridge prompt downgraded evidence（历史），现已被 isNoise 挡在晋升外，但仍占库。**待办**：找空窗用 forget/归档清掉（识别 = `downgradedFrom ∈ {profile,preferences}` 且 `isNoise(text)` 命中 AGENT_PROMPT_PATTERNS 的 events）。

### P2 观察 — minOccurrences 阈值

治本后 promote-scan 对 production 聚出 **0 真实候选** → 真实 profile/preferences 在 transcript 没高频到 minOccurrences=3，或阈值偏高。**待办**：观察是否把 minOccurrences 降到 2 让真实偏好能晋升（risk：太松又进噪声）。等 ingest 治本积累一段干净数据后重评，再决定开 `--apply`。

---

**最后更新**：2026-05-29（promote-scan capture 治本 B + bridge prompt 污染治理）
**下次回看触发**：(a) P1 主动播种实施 (b) production 库 skill 数据真有了 (c) Alice 主动想动 P2/P3 (d) Codex 主动建议 (e) 历史 bridge prompt 数据清理 / minOccurrences 阈值重评
