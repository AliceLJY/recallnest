...
- **Popularity kept out of relevance, opt-in: `RECALLNEST_POPULARITY_RANKING=bounded` and `RECALLNEST_TRIGGER_LENGTH_EXEMPT=true` (both off by default; with both unset the output is unchanged).** `bounded` replaces the three stacked popularity stages — access-count boost, hotness blend, frequency boost, each clamped at 1.0 — with one bonus at the end of the chain, `score × (1 + 0.2·h)`, `h = min(1, log2(1 + evolution.accessCount) / 5)`: it only adds, is capped at +20 %, reads one counter (the per-memory count written under the store lock; the frequency ledger, which loses updates across processes, is no longer read for ranking), and is not clamped inside the retriever — results are clamped to 1 at the `retrieve()` exit, after multi-hop merging. In this mode the full-text cut of the adaptive renderer and of the resume collapse view (L2) is 0.80 instead of 0.85. `TRIGGER_LENGTH_EXEMPT` gives a memory reached through a trigger a second path: the trigger score (× 0.9 on the hybrid path, as `applyTriggerFloor` does) goes through the same stages as the body up to length normalization, skips length normalization, and the higher path wins.
  ...
  - *Measured, prototype of the same design (2026-09-24 23:39 snapshot, clock frozen, production config; `b-lc-20+Ls` in the design grid — the implemented trigger path differs only for memories that are both new and vector-dominant, and the implementation's own shadow is in progress):* known-item 1st 190 → 474 of 501; canary 7 → 6 of 12 (mem0-borrow, whose target is cut to × 0.52 by the fact-check length anchor of 360); 64 of 100 real queries change their top 5, and on the 76 queries where any variant changed, a blind LLM proxy judge (13 / 13 calibration items right) puts top-5 DCG at 150.9 → 175.9, better 31 / same 31 / worse 14; results displayed as 100 % 123 → 2 of 730. `TRIGGER_LENGTH_EXEMPT` alone: known-item 190 → 203, canary unchanged, 92 of 100 top 5s unchanged.
  - *Cost:* of the 14 queries the proxy judge scores worse under `bounded`, 2 lose the answer from the top 5 (it stays in the top 10), 3 have a related memory replaced by an unrelated one, 1 answer moves from 1st to 2nd, 8 reorder within the top 5. Two results from different `retrieve()` calls that are both above 1 before the exit clamp tie at 1.0 where the resume composer merges them (1 of 730 results is above 1 on the snapshot; under legacy 119 tie at 1.0). The judge is a proxy: it cannot see what the user has been using lately, which is the part of popularity worth keeping.
