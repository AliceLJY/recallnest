...
- **Popularity kept out of relevance: `RECALLNEST_POPULARITY_RANKING` (default `bounded`) and `RECALLNEST_TRIGGER_LENGTH_EXEMPT` (default on). Both were opt-in until the formal shadow below finished; the maintainer switched them on the same day (2026-09-25). `RECALLNEST_POPULARITY_RANKING=legacy` and `RECALLNEST_TRIGGER_LENGTH_EXEMPT=false` restore the previous output byte for byte.** `bounded` replaces the three stacked popularity stages — access-count boost, hotness blend, frequency boost, each clamped at 1.0 — with one bonus at the end of the chain, `score × (1 + 0.2·h)`, `h = min(1, log2(1 + evolution.accessCount) / 5)`: it only adds, is capped at +20 %, reads one counter (the per-memory count written under the store lock; the frequency ledger, which loses updates across processes, is no longer read for ranking), and is not clamped inside the retriever — results are clamped to 1 at the `retrieve()` exit, after multi-hop merging. In this mode the full-text cut of the adaptive renderer and of the resume collapse view (L2) is 0.80 instead of 0.85. `TRIGGER_LENGTH_EXEMPT` gives a memory reached through a trigger a second path: the trigger score (× 0.9 on the hybrid path, as `applyTriggerFloor` does) goes through the same stages as the body up to length normalization, skips length normalization, and the higher path wins.
  - *Why:* the 2026-09-24 known-item baseline — a host is 1st for its own trigger in 38 % of 501 cases, and the popularity stages took the rank (open-loops "RecallNest 检索评分链", symptom C). Under legacy, 119 of 730 results returned for 100 real queries sat at exactly 1.0, so their order was decided by the hotness step, and a memory accessed once long ago scored below one never accessed (the hotness blend subtracts).
  - *Measured, the implementation itself (2026-09-24 23:39 snapshot of one production store, clock frozen, production config; figures are both switches on unless noted):* with both unset — and with `legacy` set explicitly — the output matched the pre-change code on all 1,263 compared items (order, scores, full `sources`, `omitted`, pre-clamp scores, both renderers' text), and every switch combination matched an independently written prototype result for result. Known-item 1st 190 → 474 of 501; canary 7 → 6 of 12 (mem0-borrow, whose target is cut to × 0.52 by the fact-check length anchor of 360). Real queries, blind LLM proxy judge (13 / 13 calibration items right): on the 65 of 100 queries whose top 5 changed, top-5 DCG 126.0 → 151.2 (better 32 / same 19 / worse 14); on 80 held-out queries judged after the parameters were frozen, 46 changed and DCG went 67.9 → 81.3 (better 26 / same 11 / worse 9). Results displayed as 100 %: 123 → 2 of 730. Full-text renders across the 180 queries 349 → 168, answers among them 44 → 53 and unrelated memories 179 → 53; neither renderer's token budget is reached at ≤ 10 results (peak 6,671 → 5,078 of 8,000 tokens). `TRIGGER_LENGTH_EXEMPT` alone: known-item 190 → 203, canary unchanged, top 5 unchanged for 92 of 100 and 76 of 80 queries, proxy judge flat (4 better, 5 worse, all 5 reorderings within the top 5).
  - *Cost:* of the 23 queries the proxy judge scores worse with both switches on (14, plus 9 held out), 3 lose the answer from the top 5 (it stays in the top 10, at 6th–7th), 7 have a related memory replaced by a lower-graded one, 13 reorder within the top 5. 22 memories stored in the last 7 days that the judge rates unrelated enter a top 5, lifted by the additive recency bonus that no longer sits under the popularity multipliers. In the resume collapse view one answer drops from summary to a single line and loses its content, and two long answers are cut to a 301-character summary that misses the part the query asks about. Two results from different `retrieve()` calls that are both above 1 before the exit clamp would tie at 1.0 where the resume composer merges them: 0 cases on the 80 scoped queries (legacy: 206 results at 1.0 in those merged lists). The judge is a proxy: it cannot see what the user has been using lately, which is the part of popularity worth keeping.
  - *Rejected:* exempting trigger hosts outright when the trigger beats the vector score (discontinuous — 0.7181 → 0.5172 as the vector score moves 0.79 → 0.81); flooring the length factor at `trigger / merged score` (continuous but not monotone, because recency is added before length normalization — the final score fell 0.64607725 → 0.64526644 over the same move — and it needs a new field in `sources`, which the HTTP API returns verbatim); clamping inside `finalizeResults` (multi-hop re-sorts and cuts after it, so a follow-up at 1.14 lost to a first-pass result at 1.08); `h` from last-access recency (it lifts whatever was searched most recently, the diagnosis's own searches included: 20–21 worse queries against 14); no popularity at all (highest proxy DCG, loses two canaries, and the judge cannot measure what it removes).
  - *Deliberately not here:* clamping RIF's reference score (RIF is off in production and its ratio is the right comparison on unclamped scores; tracked in open-loops); any change to the eligibility gate (0 of 730 returned results pass only because of popularity at today's thresholds); fixing the frequency ledger's lost updates (it is simply not read for ranking under `bounded`).

- **KG contradiction candidates: opt-in functional-predicate filter and evidence floor (`--predicates`, `--functional-min` / `--functional-min-subjects`, `--min-second-mentions`).** Both `conflicts kg-candidates` and `kg-compare` accept them. Passing any of them adds a `funnel` (how many groups each step leaves; unused steps are marked `skipped`) and, in JSON, `predicateStats`: per predicate, subjects, pairs, multi-valued subjects and functionality = distinct subjects ÷ distinct (subject, normalized object) pairs. With none of them the output is byte-identical to before — checked against golden files recorded from the previous code, and on one production scope, where all five plain invocations (up to 678 KB) compared byte for byte. (#14)
  - *Measured (2026-09-25, the same production scope as #13, 3,011 triples; table versions unchanged across the runs):* `--functional-min 0.8` 320 → 45 groups; adding `--min-second-mentions 2` → 0; `--predicates located_in` → 6. All 45 labelled by hand: 1 later change (a stale memory, the same `located_in` hit #13 found), 0 contradictions, 26 legitimate multi-values, 8 generic subject nouns from different memories colliding, 3 synonyms, 3 same name in different senses, 4 extraction errors.
  - *Why the filter does not fix it:* 19 of the 45 survivors are `is_a` and 8 are `requires`, which are not functional; they look functional only because the graph is sparse — most subjects carry a single extracted `is_a`. The evidence floor removes everything, including the only useful hit, whose two values are each mentioned once.
  - *Deliberately not here:* changed defaults, or any wiring into lint / retrieval / resume. The measured bottleneck is extraction — broad predicates, generic subjects that are never resolved to one referent — not the grouping.

- **KG contradiction candidates, shadow only (`conflicts kg-candidates`, `conflicts kg-compare`).** Groups `kg_triples` by `(scope, subject, predicate)` and reports the groups whose objects still differ after a minimal normalization (full-width → half-width, case, edge punctuation and whitespace) as *candidates*: the output's second line says that a multi-valued group can be a legitimate multi-value, a later change, or a real contradiction. Groups rank by the mention count of their second-most-mentioned value, then by size. `kg-compare` runs the existing memory-lint regex detector (`findContradictions`, now exported) on the same scope and lists both directions — regex pairs the grouping does not cover, split into "one memory has no triples" and "both have triples but never collide", and groups the regex does not catch. Both commands only `query()`: they open LanceDB directly rather than through `KGStore` / `MemoryStore`, whose initializers create tables and indexes. Logic in `src/kg-contradiction.ts`. (#13)
  - *Why:* the only discovery step for conflicts was that regex (a negation word on one side, a shared long word on both), which cannot see a semantic clash.
  - *Measured (2026-09-25, one production scope of 3,011 triples on the maintainer's machine; all three table versions unchanged across the runs):* 320 candidate groups, one of which has a second value backed by more than one memory. A reproducible sample (`--sample 10 --seed 1`), labelled by hand: 8 legitimate multi-values under broad predicates, 2 synonyms the normalization does not merge, 0 later changes, 0 contradictions; the top 10 by rank held 0 contradictions as well. The comparison on the same snapshot: 324 regex pairs; 11 caught by both; regex-only 11 (a memory without triples) + 302 (no collision); grouping-only 313. Nine pairs read in full from the "both" and "no collision" lists were none of them contradictions, so most of the 302 are the regex's own false positives rather than misses of the grouping.
  - *Cost:* two CLI subcommands, about 0.2 s per run on that scope; no write path.
  - *Rejected:* sheaf-cohomology detection — its guarantee only holds once the layer structure is built, which is itself heuristic, and it needs the whole triple graph while detection runs on ≤ 20 candidates; filtering by time windows — triples carry no validity period, and an unknown period cannot be read as a non-overlapping one.
  - *Deliberately not here:* any wiring into lint, retrieval or resume. On real data the precision is near zero and the bottleneck is upstream: 120 predicates dominated by `has` / `related_to` / `uses` / `is_a`, and almost every value mentioned once. The one useful hit came from a functional predicate (`located_in`: a tool recorded both in its hooks directory and in the trash after it was uninstalled — a later change, i.e. a stale memory).

- **Known-item eval: every trigger is a query with a known answer (`eval/known-item.ts`, `bun run eval:known-item`).** Each row of the `memory_triggers` side table becomes, at run time, a query (the trigger text) whose target is its host memory. The tool runs the production retrieval chain read-only — `source: "auto-recall"`, the audit logger swapped for a no-op, and it refuses a `--db` directory without `memories.lance` (`validateStoragePath` would otherwise create an empty store there) — and reports how often the host ranks 1st / top 3 / top 10, split by host age and by the host's own frequency multiplier; `compare` lists every case rescued or lost between two runs. Reports carry ids and numbers only; the trigger texts stay out of the repo and out of the reports. Logic in `src/known-item-eval.ts`.
  - *Why:* a 2026-09-24 diagnosis found a memory stored minutes earlier ranking 9th when searched with its own trigger. Across the pool the host was 1st in 99% of cases right after trigger recall and in 38% after the scoring chain: the popularity stages, mostly the frequency boost, took the rank (full account: open-loops "RecallNest 检索评分链", symptom C). This tool is the known-item half of the shadow any change to that chain now has to pass.
  - *Baseline (2026-09-24 23:39 snapshot of one production store, clock frozen at the snapshot, production retrieval config; 506 trigger rows → 501 cases, 5 rows whose host is not in the main table):* 1st 190 (38%), top 3 312 (62%), top 10 481 (96%), top 20 499, plus 2 queries the retrieval gate skips outright; hosts whose own frequency multiplier is ×1.0 are 1st in 18% of their cases, those above ×1.3 in 97%. A separate diagnostic script that instruments every scoring stage produced the same rank and final score for all 501 cases (0 mismatches), and the snapshot's three table versions were unchanged after the run.
  - *Cost:* 46 s for 501 queries on mini with the query-embedding cache warm; about 500 embedding calls on a cold cache. The cache defaults to `known-item-embed-cache.json` beside the store — it holds trigger texts, and that directory is gitignored.
  - *Rejected:* committing the cases as a fixture file (the trigger texts are the user's own colloquial phrasings); running them through `src/eval.ts` (it builds fresh components per case, and its per-case score blends several expectations where this needs one number, the host's rank); porting the diagnosis's per-stage breakdown (it replaces private retriever methods at run time — fine once, not something to maintain).
  - *Deliberately not here:* any change to scoring — the measured gap is the evidence for that change, not the change. Nor does it measure answer quality: one target per query, and the query is the trigger text itself (cosine 1.0 at the trigger table, the upper-bound case), so it is not the answer-quality judge the eval memo still lacks.
...
- **The minis-archive "unreadable source" test now also holds when the suite runs as root.** It made the source unreadable with `chmod 000`, which root ignores, so in root containers (cloud sessions) the case failed and never reached the read-failure path. It now stubs `fs.readFileSync` for that one path to throw `EDEADLK` (the iCloud-placeholder error it stands for), asserts that the stub was called with the source, and asserts that the reported error carries `EDEADLK`. Replacing the file with a directory was rejected because the implementation checks `isFile()` first and would never read it. *Checked:* 2,583 / 0 fail as root and 10 / 10 for this file as a non-root user (cloud session); on the maintainer's machine (non-root, Bun 1.3.14), making the implementation swallow the read failure turns this case red, and restoring it turns it green. Test-only change; total unchanged. (#15)

- **The docs described hybrid fusion as RRF and drew the hybrid path as the default.** The code does weighted score fusion — `vectorWeight` 0.7 / `bm25Weight` 0.3, a 5% bonus when both legs return the same memory, short-query reweighting (`fuseResults()` in `src/retriever.ts`) — and `retrieval.mode` defaults to `"vector"`, so fusion only runs when a config opts into `"hybrid"`. *Before:* "vector + BM25 + RRF" in both READMEs' architecture diagram, ROADMAP and `docs/architecture.md`, whose design table justified RRF as "parameter-free and robust across score distributions". *After:* the diagrams, the design table, ROADMAP and two comments in `src/retriever.ts` state the weighted fusion and the vector default; the trace stage keeps its historical name `rrf_fusion` for log compatibility, with a comment saying so. Text only; dated notes under `docs/research/` are left as written. (#12)
