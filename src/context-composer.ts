...
function hasDurablePreferenceRecall(results: RetrievalResult[]): boolean {
  return results.some((result) =>
    isStableCandidateUseful("preferences", result) &&
    isDurableStableScope(result.entry.scope) &&
    extractBoundaryMetadata(result.entry.metadata)?.layer !== "evidence",
  );
}
...
function isTaskCandidateUseful(category: TaskCategory, result: RetrievalResult): boolean {
  ...
  if (category === "patterns") {
    return containsAnyTerm(normalized, WORKFLOW_CUE_TERMS);
  }

  return containsAnyTerm(normalized, CASE_CUE_TERMS);
}

function selectTaskResults(category: TaskCategory, results: RetrievalResult[], limit: number): string[] {
  const filtered = results.filter((result) => isTaskCandidateUseful(category, result));
  const preferred = filtered.some((result) => isDurableMemoryScope(result.entry.scope))
    ? filtered.filter((result) => isDurableMemoryScope(result.entry.scope))
    : filtered;

  return dedupeText(
    preferred.map((result) => formatTaskResult(result)),
    limit,
  );
}
...
export async function composeResumeContext(
  ...
  const [profileResults, preferenceResultSets, entityResults, patternResults, caseResults] = await Promise.all([
    ...
    retrieveCandidates(deps.retriever, {
      category: "entities",
      query: buildStableQuery("entities", taskSeed),
      limit: Math.max(2, stableLimit),
      scope: resolvedScope,
    }),
    retrieveCandidates(deps.retriever, {
  ...
  const workflowFallbackResults = !continuityTask || patternResults.length > 0
    ? []
  ...
  const durablePreferenceRecall = hasDurablePreferenceRecall(preferenceResults);
  ...
  const pinnedContext = selectPinnedContext(pinAssets, {
    ...
    styleFocused: styleFocusedTask,
    skipForStyleTask: styleFocusedTask && durablePreferenceRecall,
  });
  ...
  const continuityFallbackPatterns = continuityTask && retrievedPatterns.length === 0 && fallbackPatterns.length === 0
    ...
    : [];
  const relevantPatterns = dedupeText([
    ...retrievedPatterns,
    ...fallbackPatterns,
    ...continuityFallbackPatterns,
  ], taskLimit);
